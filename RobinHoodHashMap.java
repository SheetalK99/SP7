...
public class RobinHoodHashMap<T extends Comparable<? super T>> {
	...
	public boolean contains(T x) {
		...
		return table[loc].element.compareTo(x)== 0 ;
		}
		else {
			return false;
		}
	...
	public int find(T x) {
		...
		do {
			...
			else if (table[loc].element.compareTo(x)==0 || max_displacement==0) {
				break;
			} else {
				loc = (loc + 1) % capacity;
				count++;
	...
	public T remove(T x) {
		...
		if (table[loc].element.compareTo(x)==0) {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		Integer eval_arr[] = new Integer[100000];
		...
		for (int i = 0; i < eval_arr.length; i++) {
			map.add((Integer) eval_arr[i]);
		}

		int o=0;
		for (int i = 0; i < eval_arr.length; i++) {
			if(map.contains((Integer) eval_arr[i])) {
				o++;
			}
		}
